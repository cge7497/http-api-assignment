...
const getBadRequest = (request, response, type, queryParams) => {
  ...
  if (type === 'text/xml') {
    let xmlResponse;
    // include the badRequest error id if showID is true (the request didn't have valid=true)
    if (showID) xmlResponse = `<response><message>${_message}</message><id>badRequest</id></response>`;
    else xmlResponse = `<response><message>${_message}</message></response>`;
...
const getUnauthorized = (request, response, type, queryParams) => {
  ...
  if (type === 'text/xml') {
    let xmlResponse;
    if (showID) xmlResponse = `<response><message>${_message}</message><id>unauthorized</id></response>`;
    else xmlResponse = `<response><message>${_message}</message></response>`;
    respondXML(request, response, statusCode, xmlResponse);
  } else {
    const responseJSON = {
